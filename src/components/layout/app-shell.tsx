...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-full min-h-screen bg-radar-cream">
            {/* Mobile Drawer */}
            ...
            {/* Área principal */}
            <div className={`flex flex-col flex-1 h-full min-h-screen transition-all duration-200 ease-[cubic-bezier(0.4,0,0.2,1)] ${isSidebarCollapsed ? 'md:pl-20' : 'md:pl-60'} bg-radar-cream`}>
                <Header onMenuOpen={() => setIsMobileMenuOpen(true)} />
                <main className="flex-1 flex flex-col px-4 md:px-8 pb-4">
                    {children}
                </main>
            </div>
        </div>
